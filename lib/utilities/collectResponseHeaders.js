...
function getReplicationStatus(statuses) {
    // If the destination site, we disregard any other site statuses.
    if (statuses.includes('REPLICA')) {
        return 'REPLICA';
    }
...
function collectResponseHeaders(objectMD, corsHeaders, versioningCfg,
  returnTagCount) {
    ...
    if (objectMD.replicationInfo &&
        ...
        const statuses = objectMD.replicationInfo.backends.map(backend => {
            const key = `x-amz-${backend.site}-replication-status`;
            responseMetaHeaders[key] = backend.status;
            return backend.status;
        });
        responseMetaHeaders['x-amz-replication-status'] =
            getReplicationStatus(statuses);
    }
    if (objectMD.replicationInfo &&
